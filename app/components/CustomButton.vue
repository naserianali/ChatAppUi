...
<template>
  <button :disabled="disabled" :type="type"
          ...
          class="flex items-center disabled disabled:bg-gray-300 justify-center w-full px-4 text-center rounded gap-2 h-8 ltr:flex-row-reverse">
    <span v-if="!loading">{{ t(labelKey) }}</span>
    <Icon v-if="!loading" :name="iconName" class="size-4"/>
