...
const { activeChatId } = storeToRefs(uiStore)
  ...
  <div class="flex flex-col h-full w-full bg-white dark:bg-gray-900">
    <header class="p-4 border-b flex items-center justify-between dark:border-gray-800">
      <div v-if="activeChatId" class="flex items-center gap-3">
        ...
        <h2 class="font-semibold text-lg dark:text-white">Chat Room</h2>
      </div>
      <div v-else class="text-gray-500 italic">No conversation selected</div>
    </header>
    ...
    <main ref="messagesContainer" class="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 dark:bg-black/20">
      ...
      <template v-else>
        ...
        >
          ...
          <div :class="['text-[10px] mt-1 flex justify-end gap-1', msg.user_id === user?.id ? 'text-blue-100' : 'text-gray-400']">
            {{ msg.created_at ? new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '' }}
            <Icon v-if="msg.user_id === user?.id" :name="msg.read_at ? 'lucide:check-check' : 'lucide:check'" class="size-3" />
          </div>
        </div>
      </template>
    </main>
